...
- **字号**：想要 80px 就写 80px，想用 `var(--t-h1)` 也行
  - **硬下限：16px**。stage 固定 1920×1080，但录屏后到 B 站 / 视频号
    会被缩到 1080p（56%）甚至手机 720p（38%），加上压缩码率——
    11-13px 的字在手机端只剩 4-7px 糊成线。**任何 text 元素不得低于
    `var(--t-min)` (= 16px)**。这是 templates 已写死的系统规则。
